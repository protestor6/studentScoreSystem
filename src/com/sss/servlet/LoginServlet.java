...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user == null) {
            // ÕËºÅÃÜÂë´íÎó£º´¥·¢¼ÆÊýºÍËø¶¨
            userService.handleLoginFail(uno);
            String msg=URLEncoder.encode("ÕËºÅ»òÃÜÂë´íÎó£¡", "UTF-8");;
            int errorCount = usersDao.getUserByUno(uno).getErrorCount();
            if(errorCount>=2&&errorCount<5)
            	msg=URLEncoder.encode("ÕËºÅ»òÃÜÂë´íÎó£¡Äã»¹ÓÐ"+(5-errorCount)+"´Î»ú»á"
            			, "UTF-8");;
            response.sendRedirect("login.html?Msg="+msg);
            return;
        }

        // 4. µÇÂ¼³É¹¦£ºÇå¿Õ´íÎó¼ÆÊýºÍËø¶¨×´Ì¬
